...
class QuestionnaireList {
    ...
    async importFile(input) {
        ...
        try {
            ...
            alert("Ce fichier n'est pas du JSON valide.");
            return;
        }
        if (document_.format !== 'jibjob.questionnaire') {
            alert("Ce fichier n'est pas un export de questionnaire JibJob.");
            return;
        }
        ...
        if (!confirm(`Importer « ${title} » ?\n\n${count} question(s). `
            + `Un nouveau questionnaire sera cree en brouillon — rien n'est ecrase.`)) return;
    ...
    async create() {
        const title = prompt('Titre du nouveau questionnaire ?');
    ...
    async duplicate(item) {
        if (!confirm(`Dupliquer « ${item.title} » ?\n\nUne copie en brouillon sera creee.`)) return;
        const data = await QAPI.post(`/api/questionnaires/${item.id}/duplicate/`, {});
        window.location.href = `/questionnaires/manage/${data.questionnaire.id}/`;
    }

    async archive(item) {
        if (!confirm(`Archiver « ${item.title} » ?\n\n`
            + `Il ne sera plus propose aux participants. Les resultats deja obtenus sont conserves.`)) return;
