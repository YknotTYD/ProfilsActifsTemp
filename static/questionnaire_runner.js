...
class QuestionnaireRunner {
    ...
    async finish() {
        ...
        const warn = left > 0
            ...
            : '';
        const ok = await QModal.confirm(`${warn}Terminer le questionnaire ?\n\n`
            + `Vos reponses seront verrouillees et votre resultat calcule.`,
            { title: 'Terminer le questionnaire', confirmText: 'TERMINER' });
        if (!ok) return;
