...
class QuestionnaireEditor {
    ...
    async confirmAction(action, title, detail) {
        if (!(await QModal.confirm(detail, { title, confirmText: 'CONFIRMER' }))) return;
        try {
    ...
    async remove(question) {
        const ok = await QModal.confirm(`Supprimer la question « ${question.text} » ?`,
            { title: 'Supprimer la question', confirmText: 'SUPPRIMER', danger: true });
        if (!ok) return;
        try {
            ...
            await QModal.alert(error.message, 'Erreur');
        }
    }
    ...
    async publish() {
        ...
        const ok = await QModal.confirm(lines.slice(1).join('\n'),
            { title: lines[0], confirmText: 'PUBLIER' });
        if (!ok) return;
    ...
    async statusAction(action, title, detail) {
        const ok = await QModal.confirm(detail, { title, confirmText: 'CONFIRMER' });
        if (!ok) return;
    ...
    async restore(version) {
        const ok = await QModal.confirm(`Restaurer la version ${version.version_number} ?\n\n`
            + `Une nouvelle version sera creee a partir de son contenu. L'ancienne reste intacte.`,
            { title: 'Restaurer la version', confirmText: 'RESTAURER' });
        if (!ok) return;
        const d = await this.guard(
            () => QAPI.post(`${this.base()}/versions/${version.version_number}/restore/`, {}),
            'Version restauree');
        await QModal.alert(`Version ${d.version.version_number} creee.`, 'Version restauree');
        await this.load();
    }

    async invalidate(version) {
        const reason = await QModal.prompt(
            "Elle n'acceptera plus de reponse. Les tentatives deja passees restent consultables.",
            { title: 'Pourquoi invalider cette version ?', confirmText: 'INVALIDER' });
