...
/* --------------------------------------------------------------------- */
/* Modales - remplace alert()/confirm()/prompt() par le style du site,   */
/* meme boite que la modale de deconnexion (main.html).                  */
/* --------------------------------------------------------------------- */

const QModal = (() => {

    function open(bodyChildren) {
        const overlay = qEl('div', { class: 'modal-overlay q-modal-overlay', style: 'display:flex' }, [
            qEl('div', { class: 'modal-card q-modal-pop' }, bodyChildren),
        ]);
    ...
    function focusFirst(overlay, selector) {
        requestAnimationFrame(() => overlay.querySelector(selector)?.focus());
    }

    function dialog({ title, message, body, primaryText, primaryDanger, showCancel, cancelText, onPrimary }) {
        return new Promise((resolve) => {
            ...
            const onKeydown = (e) => {
                ...
                if (e.key === 'Enter' && e.target.tagName !== 'TEXTAREA') {
                    e.preventDefault();
                    onPrimary ? onPrimary(finish) : finish(true);
                }
            };

            overlay = open([
                qEl('h2', { class: 'font-title font-extrabold text-2xl text-primary', text: title }),
                message ? qEl('p', { class: 'mt-4 text-[17px] text-text', style: 'white-space:pre-line', text: message }) : null,
                body || null,
                qEl('div', { class: 'mt-8 flex items-center justify-end gap-6' }, [
                    showCancel ? qEl('button', {
                        type: 'button', class: 'btn-ghost', text: cancelText || 'ANNULER',
                        onclick: () => finish(false),
                    }) : null,
                    qEl('button', {
                        type: 'button',
                        class: primaryDanger ? 'q-btn q-danger' : 'btn-primary-sm px-6 py-3 text-sm',
                        text: primaryText || 'OK',
                        onclick: () => onPrimary ? onPrimary(finish) : finish(true),
                    }),
                ]),
            ]);
            ...
            overlay.tabIndex = -1;
            focusFirst(overlay, 'input, button.btn-primary-sm, button.q-danger');
        });
    }

    /* Remplace window.alert(message). */
    function alertModal(message, title = 'Information') {
        return dialog({ title, message, primaryText: 'OK', showCancel: false }).then(() => undefined);
    }

    /* Remplace window.confirm(message) -> Promise<boolean>. */
    function confirmModal(message, { title = 'Confirmer', confirmText = 'CONFIRMER', cancelText = 'ANNULER', danger = false } = {}) {
        return dialog({ title, message, primaryText: confirmText, primaryDanger: danger, showCancel: true, cancelText });
    }

    /* Remplace window.prompt(message) -> Promise<string|null>. */
    function promptModal(message, { title = 'Information demandee', defaultValue = '', confirmText = 'VALIDER', placeholder = '' } = {}) {
        const input = qEl('input', { type: 'text', value: defaultValue, placeholder });
        return dialog({
            title, body: input, primaryText: confirmText, showCancel: true,
            onPrimary: (finish) => finish(input.value),
        }).then((result) => result === false ? null : input.value);
    }

    return { alert: alertModal, confirm: confirmModal, prompt: promptModal };
})();
