...
class WithdrawalTests(TestCase):
    ...
    def test_a_withdrawn_profile_disappears_from_the_recruiter_feed(self):
        add_video(self.profile, status = c.VIDEO_PUBLISHED,
                  file_url = "https://www.youtube.com/embed/x")
        request = RequestFactory().get("/")
        request.user = make_recruiter()

        def in_feed():
            return any(v["candidate"]["profile_url"].endswith("/retire/") for v in get_videos(request))
...
class ConsultationLogTests(TestCase):
    ...
    def test_a_new_consultation_notifies_the_owner_with_the_organisation_only(self):
        from profils.notifications.models import Notification

        self._visit(make_recruiter("rec_notif", organisation = "Fonderie du Val"))

        notif = Notification.objects.get(recipient = self.owner)
        self.assertEqual(notif.type, "PROFILE_CONSULTED")
        self.assertEqual(notif.url, "/profiles/me/consultations/")
        self.assertEqual(notif.payload, {"organisation": "Fonderie du Val"})
        # aucun identifiant de personne dans la notification
        self.assertNotIn("rec_notif", json.dumps(notif.payload))

    def test_a_deduplicated_consultation_does_not_re_notify(self):
        from profils.notifications.models import Notification

        recruiter = make_recruiter("rec_dedup", organisation = "Meme Boite")
        self._visit(recruiter)
        self._visit(recruiter)
